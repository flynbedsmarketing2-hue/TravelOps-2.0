...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const canCreatePackage = currentUser?.role === 'administrator' || currentUser?.role === 'travel_designer';
  const canEditPackage = currentUser?.role === 'administrator' || currentUser?.role === 'travel_designer';
  const canDeletePackage = currentUser?.role === 'administrator';
  const canDuplicatePackage = currentUser?.role === 'administrator' || currentUser?.role === 'travel_designer';
      ...
      <div className="flex flex-col max-w-[1200px] w-full gap-8">
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                {/* Stats implementation same as before... */}
                <div 
            ...
            <div className="flex p-1 bg-slate-100 rounded-lg">
                {['all', 'published', 'draft'].map(status => (
                    <button 
                        ...
                        className={`px-4 py-1.5 rounded-md text-sm font-bold capitalize transition-all ${statusFilter === status ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
        ...
        <div className="flex flex-col gap-4">
            {filteredPackages.length === 0 ? (
                <div className="p-16 text-center flex flex-col items-center bg-white rounded-xl border border-dashed border-slate-300">
                    ...
                    <p className="text-slate-500 font-medium">Aucun package trouvé.</p>
                </div>
            ) : (
                filteredPackages.map((pkg) => {
                    const stock = pkg.general.stock ?? 0;
                    let stockBadgeClass = "bg-green-100 text-green-700";
                    if (stock === 0) stockBadgeClass = "bg-red-100 text-red-700";
                    else if (stock <= 10) stockBadgeClass = "bg-orange-100 text-orange-700";
                        ...
                        <div key={pkg.id} className="group relative flex flex-col sm:flex-row gap-5 p-5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all items-start sm:items-center">
                            ...
                            <div className="flex w-full sm:w-auto items-center justify-between sm:flex-col sm:items-end gap-3 sm:gap-1 mt-2 sm:mt-0 pl-0 sm:pl-4 border-t sm:border-t-0 sm:border-l border-slate-100 pt-3 sm:pt-0">
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                    <button onClick={() => onEdit(pkg)} disabled={!canEditPackage} className="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-full transition-all disabled:opacity-30">
                                        <span className="material-symbols-outlined text-[20px]">edit</span>
                                    </button>
                                    <button onClick={() => onDuplicate(pkg)} disabled={!canDuplicatePackage} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-all disabled:opacity-30">
                                        <span className="material-symbols-outlined text-[20px]">content_copy</span>
                                    </button>
                                    {canGeneratePdf && (
                                        <div className="relative group/pdf">
                                            <button className="p-2 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-full transition-all">
                                            ...
                                            <div className="absolute right-0 top-full mt-1 w-32 bg-white rounded-lg shadow-xl border border-slate-100 py-1 hidden group-hover/pdf:block z-10 animate-fadeIn">
                                                <button onClick={() => generatePDF(pkg, 'b2c')} className="block w-full text-left px-4 py-2 text-xs hover:bg-slate-50">Version Client</button>
                                    ...
                                    <button onClick={() => onDelete(pkg.id!)} disabled={!canDeletePackage} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all disabled:opacity-30">
